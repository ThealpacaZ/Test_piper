...
class FASTTokenizer:
    def __init__(self, max_len: int = 256, fast_tokenizer_path: str = "physical-intelligence/fast"):
        ...
        try:
            ...
            # 如果失败，使用 ModelScope 下载的路径
            modelscope_path = "/root/.cache/modelscope/hub/models/AI-ModelScope/paligemma-3b-mix-224"
            self._fast_tokenizer = AutoProcessor.from_pretrained(modelscope_path, trust_remote_code=True,local_files_only=True)
    ...
    def extract_actions(self, tokens: np.ndarray, action_horizon: int, action_dim: int) -> np.ndarray:
        ...
        # 修复：确保传给 _fast_tokenizer.decode 的是正确格式的数据
        action_tokens_list = action_tokens.tolist()
        if isinstance(action_tokens_list, list) and len(action_tokens_list) > 0:
            # 如果是嵌套列表，展平它
            if isinstance(action_tokens_list[0], list):
                action_tokens_list = [item for sublist in action_tokens_list for item in sublist]
            # 确保都是整数
            action_tokens_list = [int(token) for token in action_tokens_list]
